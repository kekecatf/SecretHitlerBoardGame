...
var kart_konumlar = [Vector2(500,550), Vector2(300,100), Vector2(500,100), Vector2(700,100), Vector2(900,100), Vector2(100,300),Vector2(300,300), Vector2(500,300), Vector2(700,300), Vector2(900,300)]  # Kartların konumları
...
func zarf_olustur():
	...
	for i in range(len(oyuncular)):
		...
		zarf.rotation = kart_rotations[i]
...
func kartlari_oyuncuya_goster(oyuncu_index, rol):
	...
	rol_karti.asset_name = _rolKartiniAl(rol,oyuncu_index)  # Her karta rolünü atıyoruz
	...
	parti_karti.asset_name = _partiKartiniAl(rol,oyuncu_index)  # Card scriptindeki asset_name değişkenine değer ata
...
func _rolKartiniAl(rol,oyuncu_index):
	if oyuncu_index==0:
		if rol == "liberal":
...
func _partiKartiniAl(rol,oyuncu_index):
	if oyuncu_index == 0:
		if rol == "liberal":
